...
namespace PokeCord.Services
{
    public class PlayerDataService
    {
        ...
        {
            return await _playerDataCollection.Find(_ => true).SortByDescending(p => p.Experience).ToListAsync();
        }

        // Get list of players sorted by weekly experience from MongoDB
        public async Task<List<PlayerData>> GetWeeklyLeaderboardAsync()
        {
            var players = await _playerDataCollection.Find(_ => true).ToListAsync();
            return players
